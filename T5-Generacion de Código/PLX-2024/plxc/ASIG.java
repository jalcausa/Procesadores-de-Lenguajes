    ...
    public void gc(){
        if(this.dcha!=null){
            ...
            if(tipoVariable == tipoExpresion){
                ...
                //En caso de ser una condicion asignamos un entero segun sea true o false
                //Creo que esto no deberia ejecutarse nunca
                } else if(tipoVariable == TIPO.CONDICION){
                    if(tempExp.equals("true")){
                        Generador.asigna(nomVar, "1");
                    } else if(tempExp.equals("false")){
                        Generador.asigna(nomVar, "0");
                    } else {
                        //Ya se ha impreso la generacion de la condicion
                        if (this.dcha instanceof ASIG){
                            Generador.asigna(nomVar, ((ASIG)this.dcha).getNomVar());
                        } else {

                            CONDICION cond = (CONDICION)this.dcha;
                            String ett = cond.getEtt();
                            String etf = cond.getEtf();
    
                            //Asignamos el valor de uno en caso de ser una condicion cierta
                            Generador.printlabel(ett);
                            Generador.asigna(nomVar, "1");
    
                            //Vamos a la continuacion del codigo saltandonos el "else"
                            String etiqCont = Generador.nuevaLabel();
                            Generador.gotolabel(etiqCont);
    
                            //Asignamos el valor de cero en caso de ser una condicion falsa
                            Generador.printlabel(etf);
                            Generador.asigna(nomVar, "0");
    
                            //Imprimimos la etiqueta de continuacion del codigo
                            Generador.printlabel(etiqCont);
                        }
                        
                    }
                //En caso cualquier otro caso se imprime 
                // ti = texp; (hecho con dcha.gc())
                // id = ti;
                } else {
            ...
            //En caso asignar un entero a un boolean
            } else if (tipoVariable == TIPO.CONDICION && tipoExpresion == TIPO.ENTERO){
                //Generador.asigna(nomVar, tempExp);
