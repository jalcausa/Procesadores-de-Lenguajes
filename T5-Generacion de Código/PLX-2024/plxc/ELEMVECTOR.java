 ...
 * indice: String que indica la posicion del vector a la cual se accede
...
public class ELEMVECTOR extends EXPRESION{
    ...
    public ELEMVECTOR(String id, EXPRESION izq, EXPRESION dcha){
        ...
        //Si estamos tratando de acceder a x[i] y este es un vector, le damos el subtipo
    ...
    public void gc(){
        ...
        //En caso de ser una asignacion a una posicion del vector, ie, x[i]=exp
            ...
            //Asignado representa la etiqueta de la expresion
            String asignado ="";

            //EtiqAsignado representa una etiqueta ti para x[i]
            String etiqAsignado = "";

            //La etiqueta real del elemento es el x[i]
            this.temp = this.idVector+"["+this.indice+"]";

            //Procesamos la expresion que se asigna
            this.dcha.gc();

            //Comprobamos que los tipos son compatibles
            int tipoExpresion = ((EXPRESION)this.dcha).getTipo();
            int tipoElementoVector = this.tipo.getTipo();

            //En caso de poder hacer un casting se hace y si no son compatibles se lanza error
            if(tipoExpresion != tipoElementoVector){
                if(tipoExpresion == TIPO.ENTERO && tipoElementoVector == TIPO.REAL){
                    etiqAsignado = Generador.nuevaTemporal();
                    asignado = "(float) "+((EXPRESION)this.dcha).getTemp();
                    Generador.asigna(etiqAsignado, asignado);
                } else {
            ...
            //Imprimimos el caso dentro de indices distinguiendo si se ha hecho o no casting
            //previamente, en caso de no hacerse, etiqAsignado seguira siendo ""
            Generador.printlabel(labelIn);
            String asignacion = ((EXPRESION)this.dcha).getTemp();
            if(etiqAsignado==""){
                Generador.asigna(this.temp, asignacion);
            } else {
                Generador.asigna(this.temp, etiqAsignado);
            }
